...
__all__ = ['NotionSource', 'decode_property', 'identifiers_from_url']
...
def decode_property(value: Dict, names: Dict[str, str]=None):
    ...
    if kind == 'relation':
        return [
            names[x['id']] for x in value['relation'] if x.get('id') in names]
...
class NotionSource:
    ...
    def __iter__(self):
        ...
        """
        pages = self.library.get_papers()
        if self.verbose:
            print(f"📖 Read {len(pages)} papers from Notion")
    ...
    def __len__(self):
        return len(self.library.get_papers())
